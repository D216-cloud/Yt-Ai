...
export async function GET(req: NextRequest) {
  ...
  if (!code) {
    ...
    youtubeAuthUrl.searchParams.set("redirect_uri", `${process.env.NEXTAUTH_URL}/api/youtube/auth`)
  ...
  try {
    const tokenResponse = await fetch("https://oauth2.googleapis.com/token", {
      ...
      body: new URLSearchParams({
        ...
        redirect_uri: `${process.env.NEXTAUTH_URL}/api/youtube/auth`,
    ...
    if (!tokenResponse.ok) {
      ...
      return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/connect?error=token_failed`)
    }

    // Redirect back to connect page with access token
    const redirectUrl = new URL(`${process.env.NEXTAUTH_URL}/connect`)
    ...
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/connect?error=auth_failed`)
  }
}
