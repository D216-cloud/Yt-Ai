    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="flex">
        ...
        <main className="flex-1 md:ml-64 pb-20 md:pb-0">
          {activePage === "dashboard" && <DashboardView stats={stats} isLoading={isLoading} />}
          ...
          {navLinks.map((link) => {
            ...
            return (
              <button
                key={link.id}
                onClick={() => handleNavClick(link.id)}
                className={`flex flex-col items-center space-y-1 p-3 rounded-lg transition text-xs font-medium ${
                  activePage === link.id ? "text-blue-700" : "text-gray-600 hover:text-gray-900"
                }`}
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        <div className="bg-white border border-gray-200 rounded-xl md:rounded-2xl p-4 md:p-6 backdrop-blur-sm shadow-sm hover:shadow-md transition">
          ...
          <div className="space-y-2 md:space-y-3">
            ...
            <QuickActionButton icon={Zap} label="AI Suggestions" color="from-orange-500 to-orange-600" />
