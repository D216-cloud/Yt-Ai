...
export default function ConnectPage() {
  ...
  const [authMethod, setAuthMethod] = useState<"google" | "id" | null>(null)
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 flex flex-col md:flex-row md:items-center md:justify-center md:py-12">
        ...
        <div className="w-full md:max-w-6xl md:mx-auto flex flex-col md:flex-row md:gap-8 md:px-8">
          ...
          <div className={`w-full ${showLock ? "md:w-1/2" : "md:w-1/2"} flex flex-col p-4 md:p-8 overflow-y-auto`}>
          {showLock ? (
              ...
              <div className="space-y-6 max-w-md mx-auto w-full">
                <div>
                  ...
                  <p className="text-gray-600 text-sm md:text-lg">
                    Complete the unlock sequence to access your dashboard
                  </p>
                </div>

                <div className="space-y-3">
                  <div className="flex items-start gap-3 p-4 md:p-5 rounded-lg bg-blue-50 border border-blue-200 hover:bg-blue-100 transition">
                    <Lock className="w-5 h-5 md:w-6 md:h-6 text-blue-600 flex-shrink-0 mt-1" />
                    <div>
                      <p className="font-semibold text-gray-900 mb-1 text-sm md:text-base">Press any key to start</p>
                      <p className="text-gray-600 text-xs md:text-sm">Begin the unlock sequence by pressing any key</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3 p-4 md:p-5 rounded-lg bg-purple-50 border border-purple-200 hover:bg-purple-100 transition">
                    <ChevronRight className="w-5 h-5 md:w-6 md:h-6 text-purple-600 flex-shrink-0 mt-1" />
                    <div>
                      <p className="font-semibold text-gray-900 mb-1 text-sm md:text-base">Press Enter or Click Button</p>
                      <p className="text-gray-600 text-xs md:text-sm">Press Enter or click the unlock button to proceed</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-col justify-center h-full md:h-auto md:min-h-[500px]">
              <div className="max-w-md mx-auto w-full">
                ...
                <div className="space-y-4">
                  <h2 className="text-lg md:text-xl font-bold text-gray-900">Select Connection Method</h2>

                  {/* Google OAuth Button */}
                  <button
                    onClick={() => setAuthMethod("google")}
                    disabled={isAuthLoading}
                    className={`w-full p-4 md:p-5 rounded-lg border-2 transition-all shadow-sm hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed ${
                      authMethod === "google"
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="text-left flex-1 min-w-0">
                        ...
                        <p className="text-gray-600 text-xs md:text-sm">Fastest and most secure method</p>
                      </div>
                      <ChevronRight
                        className={`w-5 h-5 md:w-6 md:h-6 flex-shrink-0 transition-transform ${authMethod === "google" ? "rotate-90 text-blue-500" : "text-gray-400"}`}
                      />
                    </div>
                  </button>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="text-left flex-1 min-w-0">
                        <h3 className="font-semibold text-gray-900 mb-1 text-sm md:text-base">Channel ID</h3>
                        <p className="text-gray-600 text-xs md:text-sm">Enter your YouTube Channel ID</p>
                      </div>
                      <ChevronRight
                        className={`w-5 h-5 md:w-6 md:h-6 flex-shrink-0 transition-transform ${authMethod === "id" ? "rotate-90 text-purple-500" : "text-gray-400"}`}
                      />
                    </div>
                  </button>
                  ...
                  {authMethod && (
                    <div className="bg-green-50 border border-green-300 rounded-lg p-3 md:p-4 flex items-center gap-2">
                      ...
                      <p className="text-green-700 text-xs md:text-sm font-medium">
                        {authMethod === "google" ? "Google OAuth" : "Channel ID"} selected. Secure connection.
                      </p>
                    </div>
                  )}
                </div>
          ...
          {/* Right Section - Channel Preview / Lock Animation */}
          <div
            className={`w-full ${showLock ? "md:w-1/2" : "md:w-1/2"} flex flex-col items-center justify-center p-4 md:p-8 border-t md:border-t-0 md:border-l border-gray-200`}
          >
            {showLock ? (
              ...
              <div className="bg-gradient-to-b from-gray-50 to-white rounded-lg p-6 md:p-8 w-full max-w-md mx-auto border border-gray-200 shadow-sm">
                <div className="text-center">
                  ...
                  <Button
                    ...
                    disabled={!authMethod || isAuthLoading || isLoading || !!youtubeChannel}
                  ...
                  >
                    {isAuthLoading || isLoading ? (
                      ...
                      <>{authMethod ? "Connect with YouTube" : "Select Method to Continue"}</>
                    )}
                  </Button>
